...
def contrastStretch(image, min, max):
    ...
    iO = (iI - minI) * (
        ((maxO - minO) / (maxI - minI)) + minO
    )  # image output    # for m in range(0,M):
    return iO
...
mmc.loadSystemConfiguration(
    mm_dir + "MMConfig_QCam.cfg"
)  # load your micromanager camera configuration file
mmc.setExposure(200)
mmc.snapImage()
im1 = mmc.getImage()
...
# print('-----runnit-----')
timeLog = (
    save_path + date + "_test_inference_time.txt"
)  # check the inference time from the CNN with your system
    ...
    if mmc.getRemainingImageCount() > 0:
        ...
        if use_YOLO:
            ...
            image = Image.fromarray(
                np.uint8(frame)
            )  # frame if not using contrastStretch, else alter
